...
def generate_launch_description():
    ...
    with open(yaml_file_path, 'r') as f:
        ...
        model_ns = yaml_dict["namespace"].lstrip('/') # remove barra inicial se houver
    print("namespace: ", model_ns)

    yaml_file_path_2 = os.path.join(
        get_package_share_directory('sjtu_drone_bringup'),
        'config', 'drone1.yaml'
    )   
    robot_description_config_2 = xacro.process_file(xacro_file, mappings={"params_path": yaml_file_path_2})
    robot_desc2 = robot_description_config_2.toxml()
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            arguments=[robot_desc, f"/{model_ns}"],
            output="screen"
        ),
        Node(
            package="sjtu_drone_bringup",
            executable="spawn_drone",
            arguments=[robot_desc2, "/mass"],
            output="screen"
        ),
        Node(
            package="robot_state_publisher",
            ...
            namespace="/mass",
            output="screen",
            parameters=[{"use_sim_time": use_sim_time, "robot_description": robot_desc2, "frame_prefix": "/mass/"}],
            arguments=[robot_desc2]
        ),
        Node(
            package='joint_state_publisher',
            executable='joint_state_publisher',
        ...
        Node(
            package="tf2_ros",
            executable="static_transform_publisher",
            arguments=["0", "0", "0", "0", "0", "0", "world", "/mass/odom"],
            output="screen"
        ),
    ])
